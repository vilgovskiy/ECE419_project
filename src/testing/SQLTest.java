...
public class SQLTest extends TestCase {
    ...
    @Test
    public void testSQLTableParser() {
        JsonMessage msg = new JsonMessage();
        List<String> sqlStatements = new ArrayList<>(Arrays.asList(
                "CREATE TABLE IF NOT EXISTS warehouses ( id integer PRIMARY KEY, name text NOT NULL, capacity real)",
                "CREATE TABLE warehouses ( id integer PRIMARY KEY, name text NOT NULL, capacity real)",
                "INSERT INTO warehouses (name, capacity) VALUES (\"warehouse1\", \"100\")",
                "SELECT name, capacity FROM warehouses",
                "UPDATE warehouses SET capacity=600 WHERE name='warehouse5'",
                "DROP TABLE IF EXISTS warehouses"
        ));

        for (String stmt : sqlStatements) {
            msg.setKey(stmt);
            String tableName = SQLParser.getTableFromSQLMsg(msg).get(0);
            assert(tableName.equals("warehouses"));
        }

        msg.setKey("SELECT warehouses.name, amznWarehouses.name FROM warehouses INNER JOIN amznWarehouses ON warehouses.capacity=amznWarehouses.capacity");
        String tableName = SQLParser.getTableFromSQLMsg(msg).get(0);
        String joinTableName = SQLParser.getTableFromSQLMsg(msg).get(1);
        assert(tableName.equals("warehouses"));
        assert(joinTableName.equals("amznWarehouses"));
    }
}
